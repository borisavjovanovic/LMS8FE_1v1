...
{
	char *cstr = (char *)str;
	return Lms8fe_my_write(com, cstr, len);
}

int Lms8fe_serialport_read(LMS8FE_COM com, char *buff, int len)
{
	int n = Lms8fe_my_read(com, buff, len);
	return n;
}
...
int Lms8fe_write_buffer(lms_device_t *dev, LMS8FE_COM com, unsigned char *data, int size)
{
	...
	{ // prioritize direct connection
		return Lms8fe_write_buffer_fd(com, data, size);
	}
	else if (dev != NULL)
	{
		// B.J.
...
{
	int actual_length;
	actual_length = Lms8fe_serialport_write(com, (char *)c, size);
...
int Lms8fe_read_buffer(lms_device_t *dev, LMS8FE_COM com, unsigned char *data, int size)
{
	if (com.fd >= 0)
	{ // prioritize direct connection
		return Lms8fe_read_buffer_fd(com, data, size);
	}
	else if (dev != NULL)
	{
		// B.J.
...
int Lms8fe_read_buffer_fd(LMS8FE_COM com, unsigned char *data, int size)
{
	...
	do
	{
		int count = Lms8fe_serialport_read(com, (char *)data + received, size - received);
		if (count > 0)
			received += count;
		if (received >= size)
			break;
...
int Lms8fe_Cmd_GetInfo(lms_device_t *dev, LMS8FE_COM com, Lms8fe_boardInfo *info)
{
	...
	return LMS8FE_SUCCESS;
}

// int Lms8fe_ReadConfig(const char *filename, lms8fe_boardState *stateBoard, guiState *stateGUI) {
int Lms8fe_ReadConfig(const char *filename, lms8fe_boardState *stateBoard)
{
	typedef INI<string, string, string> ini_t;
	ini_t parser(filename, true);

	//	if (parser.select("LimeLMS8FE_Board_Settings") == false)
	if (parser.select("LMS8FE_Board_Settings") == false)
		return LMS8FE_ERROR_CONF_FILE;
	/*
		...
		if (parser.select("LimeLMS8FE_GUI_Settings")) {
...
int Lms8fe_SaveConfig(const char *filename, lms8fe_boardState state)
{
	...
	fprintf(fout, "[LMS8FE_Board_Settings]\n");
...
void Lms8fe_mySleep(int sleepms)
{
#ifdef __unix__
...
int Lms8fe_Cmd_Hello(lms_device_t *dev, LMS8FE_COM com)
{
	...
	while (!connected && (attempts < LMS8FE_MAX_HELLO_ATTEMPTS))
	{
		...
		len = dwlen;
#endif
		if ((len == 1) && (buf[0] == LMS8FE_CMD_HELLO))
			connected = true;
		attempts++;
	}

	result = (connected) ? 0 : LMS8FE_ERROR_COMM;
	return result;
}
...
int Lms8fe_Cmd_LoadConfig(lms_device_t *dev, LMS8FE_COM com, const char *filename)
{
	...
	//	guiState stateGUI;
	//	result = Lms8fe_ReadConfig(filename, &state, &stateGUI);
	result = Lms8fe_ReadConfig(filename, &state);
	if (result != 0)
		return result;
...
int Lms8fe_Cmd_Select_Channel(lms_device_t *dev, LMS8FE_COM com, uint8_t channel)
{
	...
	buf[0] = LMS8FE_CMD_SELECT_CHANNEL;
	buf[1] = channel;
...
{
	return Lms8fe_Cmd_Configure(dev, com, state.channelIDRX, state.channelIDTX, state.selPortRX, state.selPortTX, state.mode, state.notchOnOff, state.attValue, state.enableSWR, state.sourceSWR);
}
*/

int Lms8fe_Cmd_Configure(lms_device_t *dev, LMS8FE_COM com, lms8fe_boardState boardState)
{
	uint8_t state[STATE_SIZE];
...
/*
int Lms8fe_Cmd_Configure(lms_device_t *dev, LMS8FE_COM com, int channelIDRX, int channelIDTX, int selPortRX, int selPortTX, int mode, int notch, int attenuation, int enableSWR, int sourceSWR) {
...
int Lms8fe_Cmd_Mode(lms_device_t *dev, LMS8FE_COM com, int mode)
{
	...
	unsigned char buf[LMS8FE_BUFFER_SIZE_MODE];
	int len;

	memset(buf, 0, LMS8FE_BUFFER_SIZE_MODE);

	buf[0] = LMS8FE_CMD_MODE;

	buf[1] = mode;

	if (Lms8fe_write_buffer(dev, com, buf, LMS8FE_BUFFER_SIZE_MODE) != 0)
		return LMS8FE_ERROR_COMM;
	len = Lms8fe_read_buffer(dev, com, buf, LMS8FE_BUFFER_SIZE_MODE);
	if (len == -1)
		return (LMS8FE_ERROR_COMM);
...
int Lms8fe_Cmd_ReadADC(lms_device_t *dev, LMS8FE_COM com, int adcID, int *value)
{
	...
	if (adcID == LMS8FE_ADC1)
		buf[0] = LMS8FE_CMD_READ_ADC1;
	else
		buf[0] = LMS8FE_CMD_READ_ADC2;
...
{
	if ((gpioNum != LMS8FE_GPIO4) & (gpioNum != LMS8FE_GPIO5))
		return LMS8FE_ERROR_GPIO_PIN;
...
{
	double sleepms = 0.5 * 1 / LMS8FE_I2C_FSCL * 1000; // for example 0.5 * period - Check this!!!
	Lms8fe_mySleep(sleepms);
}
...
{
	if (Lms8fe_i2c_setVal(lms, GPIO_SDA, 1) != 0)
		return -1;
	Lms8fe_i2c_setVal(lms, GPIO_SCL, 1);
	Lms8fe_i2c_setVal(lms, GPIO_SDA, 0);
	Lms8fe_i2c_setVal(lms, GPIO_SCL, 0);
	return 0;
}

int Lms8fe_i2c_stop(lms_device_t *lms)
{
	if (Lms8fe_i2c_setVal(lms, GPIO_SDA, 0) != 0)
		return -1;
	Lms8fe_i2c_setVal(lms, GPIO_SCL, 1);
	Lms8fe_i2c_setVal(lms, GPIO_SDA, 1);
	return 0;
}
...
int Lms8fe_i2c_rx(lms_device_t *lms, char ack, unsigned char *d)
{
	...
	for (x = 0; x < 8; x++)
	{
		...
		} while (Lms8fe_i2c_getVal(lms, GPIO_SCL) == 0); // wait for any SCL clock stretching
		Lms8fe_i2c_dly();
		if (Lms8fe_i2c_getVal(lms, GPIO_SDA))
			(*d) |= 1;
		Lms8fe_i2c_setVal(lms, GPIO_SCL, 0);
	}
	if (ack)
...
int Lms8fe_i2c_tx(lms_device_t *lms, unsigned char d)
{
	...
	Lms8fe_i2c_setVal(lms, GPIO_SCL, 1);
	Lms8fe_i2c_dly();
	b = Lms8fe_i2c_getVal(lms, GPIO_SDA); // possible ACK bit
	Lms8fe_i2c_setVal(lms, GPIO_SCL, 0);
	return b;
}
...
{
	unsigned char addressI2C = LMS8FE_I2C_ADDRESS;
	unsigned char addressByte = addressI2C << 1;
	unsigned char addressByteW = addressByte & ~1;

	if (Lms8fe_i2c_start(lms) != 0) // send start sequence
		return -1;
	Lms8fe_i2c_tx(lms, addressByteW); // I2C address with R/W bit clear

	for (int i = 0; i < size; i++)
	{
		Lms8fe_i2c_tx(lms, c[i]);
	}

	Lms8fe_i2c_stop(lms); // send stop sequence
...
int Lms8fe_i2c_read_buffer(lms_device_t *lms, unsigned char *c, int size)
{
	...
	Lms8fe_i2c_start(lms);			  // send a restart sequence
	Lms8fe_i2c_tx(lms, addressByteR); // I2C address with R/W bit set
	...
	for (i = 0; i < size; i++)
	{
		...
		if (i == (size - 1))
			ack = 0;
		int res = Lms8fe_i2c_rx(lms, ack, &(c[i]));
		if (res != LMS8FE_SUCCESS)
			return LMS8FE_ERROR_COMM;
	}
	Lms8fe_i2c_stop(lms); // send stop sequence
	return i;
...
int Lms8fe_Cmd_SC1905_SPI_Message_Memory(lms_device_t *dev, LMS8FE_COM com, uint16_t address, uint8_t *val, bool isRead, int bytesNo, bool isEEPROM)
{
	...
	buf[0] = LMS8FE_CMD_SC1905_SPI_MESSAGE_MEMORY;
...
int Lms8fe_Cmd_SC1905_SPI_Special_Command(lms_device_t *dev, LMS8FE_COM com, int command)
{
	...
	buf[0] = LMS8FE_CMD_SC1905_SPI_SPECIAL_COMMAND;
	...
	switch (command)
	{
	case LMS8FE_SC1905_COMMAND_CLEAR_WARNING:
		buf[2] = 0x03;
		reply[2] = 0x03;
		break;
	case LMS8FE_SC1905_COMMAND_ACTIVATE_OUTPUTS:
		buf[2] = 0x04;
		reply[2] = 0x04;
		break;
	case LMS8FE_SC1905_COMMAND_WRITE_MAXPWRCALPARAM_A:
		buf[2] = 0xf5;
		reply[2] = 0xf5;
		break;
	case LMS8FE_SC1905_COMMAND_WRITE_MAXPWRCALPARAM_B:
		buf[2] = 0xf6;
		reply[2] = 0xf6;
		break;
	case LMS8FE_SC1905_COMMAND_CLEAR_MAXPWRCALPARAM_A:
		buf[2] = 0xf3;
		reply[2] = 0xf3;
		break;
	case LMS8FE_SC1905_COMMAND_CLEAR_MAXPWRCALPARAM_B:
		buf[2] = 0xf4;
		reply[2] = 0xf4;
		break;
	default:
		return LMS8FE_ERROR_SC1905_SPECIAL_MESSAGE_UNKNOWN;
	}
	...
	{
		result = LMS8FE_ERROR_SC1905_SPECIAL_MESSAGE_WRONG_REPLY;
	}
	//	return result;
	return result;
}
...
int Lms8fe_Cmd_SC1905_Apply_Frequency(lms_device_t *dev, LMS8FE_COM com, int freqRange, int minFreq, int maxFreq)
{
	...
	result = Lms8fe_Cmd_SC1905_SPI_Message_Memory(dev, com, address, val, false, 2);
	if (result != LMS8FE_SUCCESS)
		return result;
...
int Lms8fe_Cmd_Set_Config_Full(lms_device_t *dev, LMS8FE_COM com, uint8_t *state, int size)
{
	...
	buf[0] = LMS8FE_CMD_SET_CONFIG_FULL;
...
int Lms8fe_Cmd_Get_Config_Full(lms_device_t *dev, LMS8FE_COM com, uint8_t *state, int size)
{
	...
	buf[0] = LMS8FE_CMD_GET_CONFIG_FULL;
