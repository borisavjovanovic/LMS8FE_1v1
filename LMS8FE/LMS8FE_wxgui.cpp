...
void LMS8FE_wxgui::Initialize(lms_device_t *lms)
{
	lmsControl = lms;
...
void LMS8FE_wxgui::OnbtnOpenPort(wxCommandEvent &event)
{
	...
	{
		if (GetCommType() == LMS8FE_USB)
		{
			lms8fe = LMS8FE_Open(PortName.mb_str(), lmsControl);
			/*
						char port[100] = "COM3";
	...
	//	strcpy(buf, "asdf");
		...
		buf[3] = 'f';
		buf[4] = '\0';
	//	int res = write_buffer(dev->sdrDevice, dev->com, buf, LMS8FE_BUFFER_SIZE);
		DWORD bytesSend, bytesRead;

		//Try to write the buffer on the Serial port
	//	bool res = WriteFile(dev->com.hComm, buf, LMS8FE_BUFFER_SIZE, &bytesSend, NULL);
	//	bool res = WriteFile(hCom, buf, LMS8FE_BUFFER_SIZE, &bytesSend, NULL);
	...
	//	res = ReadFile(hCom, buf, LMS8FE_BUFFER_SIZE, &bytesRead, NULL);

		Sleep(2000);

		res = ReadFile(dev->com.hComm, buf, LMS8FE_BUFFER_SIZE, &bytesRead, NULL);
		len = bytesRead;
	//	len = read_buffer(dev->sdrDevice, dev->com, buf, LMS8FE_BUFFER_SIZE);
	...
	*/
	...
	{
		AddMssg("Error initializing serial port");
		AddMssg("Please check the connection between the SDR and LimeLMS8FE");
		LMS8FE_Close(lms8fe);
		lms8fe = nullptr;
		return;
	}
...
void LMS8FE_wxgui::stopTimerSC1905UpdateThread()
{
	threadID++;
	std::this_thread::sleep_for(std::chrono::milliseconds(2000));
}
...
void LMS8FE_wxgui::AddMssg(const char *mssg)
{
	wxString s(mssg, wxConvUTF8);
	AddMssg(s);
}

void LMS8FE_wxgui::AddMssg(const wxString &mssg)
{
	time_t rawtime;
	struct tm *timeinfo;
	char buffer[80];
...
void LMS8FE_wxgui::OnbtnRefreshPorts(wxCommandEvent &event)
{
	ReadPorts();
}

void LMS8FE_wxgui::OnbtnReset(wxCommandEvent &event)
{

	int result = LMS8FE_Reset(lms8fe);
...
void LMS8FE_wxgui::OnbtnOpen(wxCommandEvent &event)
{
	wxFileDialog dlg(this, _("Open config file"), "", "", "Project-File (*.ini)|*.ini", wxFD_OPEN | wxFD_FILE_MUST_EXIST);
	if (dlg.ShowModal() == wxID_CANCEL)
		return;

	int result = LMS8FE_LoadConfig(lms8fe, dlg.GetPath().To8BitData());
	if (result != LMS8FE_SUCCESS)
	{
		PrintError(result);
		return;
...
void LMS8FE_wxgui::State2GUI(lms8fe_boardState state)
{
}

void LMS8FE_wxgui::GUI2State(lms8fe_boardState *state)
{
}

void LMS8FE_wxgui::OnbtnBoard2GUI(wxCommandEvent &event)
{
	//	State2GUI(state);
}

void LMS8FE_wxgui::PrintError(int errorCode)
{
	switch (errorCode)
	{
	case LMS8FE_ERROR_COMM:
		AddMssg("ERROR: Communication error.");
		break;
	case LMS8FE_ERROR_TX_CONN:
		AddMssg("ERROR: Wrong TX connector.");
		break;
	case LMS8FE_ERROR_RX_CONN:
		AddMssg("Error: Wrong RX connector.");
		break;
	case LMS8FE_ERROR_RXTX_SAME_CONN:
		AddMssg("ERROR: Mode RX & TX not allowed when RX and TX are on the same port.");
		break;
	case LMS8FE_ERROR_CELL_WRONG_MODE:
		AddMssg("ERROR: Wrong mode for cellular channel.");
		break;
	case LMS8FE_ERROR_CELL_TX_NOT_EQUAL_RX:
		AddMssg("ERROR: For cellular channels RX and TX channels must be the same.");
		break;
	case LMS8FE_ERROR_WRONG_CHANNEL_CODE:
		AddMssg("ERROR: Wrong channel code.");
		break;
	case LMS8FE_ERROR_CONF_FILE:
		AddMssg("ERROR: Problem with .ini configuration file.");
		break;
	case LMS8FE_ERROR_GPIO_PIN:
		AddMssg("ERROR: Non-configurable GPIO pin specified. Only pins 4 and 5 are configurable.");
		break;
	case LMS8FE_ERROR_COMM_SYNC:
		AddMssg("ERROR: Communication synchronization error.");
		break;

	case LMS8FE_ERROR_SC1905_RSR:
		AddMssg("Error: SC1905 RSR Read Error.");
		break;

	case LMS8FE_ERROR_SC1905_CHK:
		AddMssg("Error: SC1905 CHK Read Error.");
		break;

	case LMS8FE_ERROR_SC1905_SPECIAL_MESSAGE_UNKNOWN:
		AddMssg("Error: Unknown or unimplemented SC1905 Special Message required.");
		break;

	case LMS8FE_ERROR_SC1905_SPECIAL_MESSAGE_WRONG_REPLY:
		AddMssg("Error: SC1905 provided wrong reply to Special Message sent.");
		break;

	case LMS8FE_ERROR_SC1905_EEPROM_BUSY:
		AddMssg("Error: SC1905 EEPROM Busy.");
		break;

	case LMS8FE_ERROR_SC1905_EEPROM_LOCKED:
		AddMssg("Error: SC1905 EEPROM Locked.");
		break;

	case LMS8FE_ERROR_SC1905_EEPROM_UNLOCKED:
		AddMssg("Error: SC1905 EEPROM Not Locked.");
		break;

	case LMS8FE_ERROR_SC1905_MAXPWRCLEARONGOING_TOO_LONG:
...
void LMS8FE_wxgui::OnrbI2CrbUSB(wxCommandEvent &event)
{
	UpdateLMS8FEForm();
}

void LMS8FE_wxgui::UpdateLMS8FEForm()
{
...
void LMS8FE_wxgui::OnQuit(wxCommandEvent &event)
{
...
void LMS8FE_wxgui::OntbtnDiode(wxCommandEvent& event)
{
	...
	//	int state = 1;
	int result = LMS8FE_Diode(lms8fe, state);
	if (result != LMS8FE_SUCCESS)
	{
		PrintError(result);
		return;
...
void LMS8FE_wxgui::OntbtnDiodeSPI(wxCommandEvent &event)
{
	int state = tbtnDiodeSPI->GetValue();
	//	int state = 1;
	int result = LMS8FE_DiodeSPI(lms8fe, state);
	if (result != LMS8FE_SUCCESS)
	{
		PrintError(result);
		return;
...
void LMS8FE_wxgui::OnbtnSC1905_Read_FW_Version(wxCommandEvent &event)
{
	...
	uint8_t val = 0x00;
	// int LMS8FE_SC1905_SPI_Message_Memory(lms8fe_dev_t * lms8fe, uint16_t address, uint8_t * val, bool isRead, int bytesNo);
	int result = LMS8FE_SC1905_SPI_Message_Memory(lms8fe, address, &val, true, 1);
...
void LMS8FE_wxgui::OnbtnSC1905_Read_Product_ID(wxCommandEvent &event)
{
	uint16_t address = 0x959;
	uint8_t val[2];
	// int LMS8FE_SC1905_SPI_Message_Memory(lms8fe_dev_t * lms8fe, uint16_t address, uint8_t * val, bool isRead, int bytesNo);
	int result = LMS8FE_SC1905_SPI_Message_Memory(lms8fe, address, val, true, 2);
...
void LMS8FE_wxgui::UpdateSC1905(bool full)
{
	uint16_t address;
	uint8_t val[2];
	int result;
	...
	address = 0x959;
	result = LMS8FE_SC1905_SPI_Message_Memory(lms8fe, address, val, true, 2);
	...
	// Read FW Version
	address = 0x003;
	result = LMS8FE_SC1905_SPI_Message_Memory(lms8fe, address, val, true, 1);
	...
	// It is not clear at this moment if the EPROM control will be needed for proper LMS8FE operation...
	address = 0xFC1B;
	//	address = 0xFC00;
	result = LMS8FE_SC1905_SPI_Message_Memory(lms8fe, address, val, true, 2, true);
	...
	address = 0xfc62;
	result = LMS8FE_SC1905_SPI_Message_Memory(lms8fe, address, val, true, 1, true);
	...
	for (int i = 0; i < SC1905_EEPROM_PARAMETERS_ROWS; i++)
	{
		...
		result = LMS8FE_SC1905_SPI_Message_Memory(lms8fe, address, val, true, bytesNo, true);
...
void LMS8FE_wxgui::OnbtnSC1905_Force_Update(wxCommandEvent &event)
{
	UpdateSC1905(true);
}

void LMS8FE_wxgui::OnbtnSC1905_Reset_RFPAL(wxCommandEvent &event)
{
	int result = LMS8FE_SC1905_Reset(lms8fe);
...
void LMS8FE_wxgui::timerSC1905UpdateStatusThreadFunction(float interval, int id, std::atomic<int> &currID)
{
...
// ovdi!!!
void LMS8FE_wxgui::UpdateSC1905Status()
{
	uint16_t address;
	uint8_t val[2];
	int result;
...
void LMS8FE_wxgui::OnchSC1905_Update_Rate(wxCommandEvent &event)
{
	...
	else
	{
		...
		//		interval = 1;
		//		timerSC1905Update = new std::thread(&LMS8FE_wxgui::timerSC1905UpdateStatusThreadFunction, this, interval, (int)threadID, std::ref(threadID));
		//		timerSC1905Update->detach();
		btnSC1905_Force_Update->Show();
	}
}

void LMS8FE_wxgui::OnbtnSC1905_Apply_Frequency(wxCommandEvent &event)
{
	...
	result = LMS8FE_SC1905_Apply_Frequency(lms8fe, freqRange, (int)minFreq, (int)maxFreq);
...
void LMS8FE_wxgui::OnchSC1905_Frequency_Range(wxCommandEvent &event)
{
	int freqRangeIndex = chSC1905_Frequency_Range->GetSelection();
	wxString strTmp;
...
void LMS8FE_wxgui::OnchSC1905_Set_Duty_Cycle_Feedback(wxCommandEvent &event)
{
	int result = LMS8FE_SUCCESS;
	uint16_t address;
	...
	address = 0x017;
	result = LMS8FE_SC1905_SPI_Message_Memory(lms8fe, address, val, false, 1);
}

void LMS8FE_wxgui::OnchSC1905_Set_Adaptation_State(wxCommandEvent &event)
{
	int result = LMS8FE_SUCCESS;
	uint16_t address;
	uint8_t val[2] = {0x00, 0x00};
...
void LMS8FE_wxgui::OnchSC1905_Set_Correction_Enable(wxCommandEvent &event)
{
	int result = LMS8FE_SUCCESS;
	uint16_t address;
	uint8_t val[2] = {0x00, 0x00};
	...
	result = LMS8FE_SC1905_SPI_Special_Command(lms8fe, LMS8FE_SC1905_COMMAND_ACTIVATE_OUTPUTS);
	if (result != LMS8FE_SUCCESS)
	{
		PrintError(result);
		return;
...
void LMS8FE_wxgui::OnbtnSC1905_Clear_Warnings(wxCommandEvent &event)
{
	int result = LMS8FE_SUCCESS;
	result = LMS8FE_SC1905_SPI_Special_Command(lms8fe, LMS8FE_SC1905_COMMAND_CLEAR_WARNING);
	if (result != LMS8FE_SUCCESS)
	{
		PrintError(result);
		return;
...
void LMS8FE_wxgui::OnKillFocustxtCtrlSC1905_RFIN_Offset(wxFocusEvent &event)
{
	event.Skip();
	...
	if (dialog_return_value == wxID_YES)
	{
		...
		address = 0xfc19;
		int result = LMS8FE_SC1905_SPI_Message_Memory(lms8fe, address, val, false, 2, true);
		if (result != LMS8FE_SUCCESS)
		{
			PrintError(result);
			return;
...
void LMS8FE_wxgui::OnKillFocustxtCtrlSC1905_RFFB_Offset(wxFocusEvent &event)
{
	event.Skip();
	int dialog_return_value = wxID_NO; // initialize to a sane default value
...
void LMS8FE_wxgui::OnbtnSC1905_Calibrate_RFIN_PMU(wxCommandEvent &event)
{
	uint16_t address;
	uint8_t val[2] = {0x00, 0x00};
...
void LMS8FE_wxgui::OnbtnSC1905_Calibrate_RFFB_PMU(wxCommandEvent &event)
{
	uint16_t address;
	uint8_t val[2] = {0x00, 0x00};
...
void LMS8FE_wxgui::SC1905_Set_Cal_Param(int freq)
{
	int command;
	...
	if (freq == LMS8FE_SC1905_FREQUENCY_A)
	{
		command = LMS8FE_SC1905_COMMAND_WRITE_MAXPWRCALPARAM_A;
		//		address = 0xdc4;
	}
	if (freq == LMS8FE_SC1905_FREQUENCY_B)
	{
		command = LMS8FE_SC1905_COMMAND_WRITE_MAXPWRCALPARAM_B;
		//		address = 0xdc6;
	}

	int result = LMS8FE_SC1905_SPI_Special_Command(lms8fe, command);
	if (result != LMS8FE_SUCCESS)
	{
		PrintError(result);
		return;
	...
	std::thread *maxPwrCalOngoingThread = new std::thread(&LMS8FE_wxgui::maxPwrCalOngoingThreadFunction, this, freq, std::ref(userCancel), &cancelDialog);
...
void LMS8FE_wxgui::OnbtnSC1905_Set_Cal_Param_A(wxCommandEvent &event)
{
	SC1905_Set_Cal_Param(LMS8FE_SC1905_FREQUENCY_A);
}

void LMS8FE_wxgui::OnbtnSC1905_Set_Cal_Param_B(wxCommandEvent &event)
{
	SC1905_Set_Cal_Param(LMS8FE_SC1905_FREQUENCY_B);
}

void LMS8FE_wxgui::maxPwrCalOngoingThreadFunction(int freq, std::atomic<bool> &userCancel, CancelDialog *cancelDialog)
{
	//	int i = 0;
	//	for (i = 0; i < 5; i++) {
...
void LMS8FE_wxgui::OnbtnSC1905_Clear_Cal_Param(wxCommandEvent &event)
{
	SC1905_Clear_Cal_Param(LMS8FE_SC1905_FREQUENCY_A);
	SC1905_Clear_Cal_Param(LMS8FE_SC1905_FREQUENCY_B);
}

void LMS8FE_wxgui::SC1905_Clear_Cal_Param(int freq)
{
	int command;
	...
	if (freq == LMS8FE_SC1905_FREQUENCY_A)
	{
		command = LMS8FE_SC1905_COMMAND_CLEAR_MAXPWRCALPARAM_A;
	}
	if (freq == LMS8FE_SC1905_FREQUENCY_B)
	{
		command = LMS8FE_SC1905_COMMAND_CLEAR_MAXPWRCALPARAM_B;
	}
	...
	for (int i = 0; i < LMS8FE_SC1905_MAXPWRCLEARONGOING_ATTEMPTS; i++)
	{
		result = LMS8FE_SC1905_SPI_Message_Memory(lms8fe, address, val, true, 1);
		if (result != LMS8FE_SUCCESS)
		{
	...
	if (!done)
	{
		PrintError(LMS8FE_ERROR_SC1905_MAXPWRCLEARONGOING_TOO_LONG);
	}
}

void LMS8FE_wxgui::OnchSC1905_Guard_Band(wxCommandEvent &event)
{
	...
	val[0] = selection;
	address = 0xfc62;
	int result = LMS8FE_SC1905_SPI_Message_Memory(lms8fe, address, val, false, 1, true);
	if (result != LMS8FE_SUCCESS)
	{
		PrintError(result);
		return;
...
void LMS8FE_wxgui::OnrbtnSC1905_EEPROM_Write(wxCommandEvent &event)
{
	SC1905_EEPROM_Update_Form();
}

void LMS8FE_wxgui::OnrbtnSC1905_EEPROM_Read(wxCommandEvent &event)
{
	SC1905_EEPROM_Update_Form();
}

void LMS8FE_wxgui::OnbtnSC1905_EEPROM_Execute(wxCommandEvent &event)
{
	uint16_t address;
	...
	*/
	...
	int result = LMS8FE_SC1905_SPI_Message_Memory(lms8fe, address, val, isRead, bytesNo, true);
...
void LMS8FE_wxgui::SC1905_EEPROM_Update_Form()
{
	txtCtrlSC1905_EEPROM_Value->Enable(rbtnSC1905_EEPROM_Write->GetValue());
}

void LMS8FE_wxgui::OnGridCellChangegridSC1905_EEPROM_Parameters(wxGridEvent &event)
{
	...
	int result = LMS8FE_SC1905_SPI_Message_Memory(lms8fe, address, val, false, bytesNo, true);
...
void LMS8FE_wxgui::str2bytes(wxString valueString, int selection, uint8_t *val)
{
	uint8_t value_uint8 = 0x00;
	int8_t value_int8 = 0x00;
	uint16_t value_uint16 = 0x00;
...
void LMS8FE_wxgui::bytes2str(uint8_t *val, int selection, long *valLong)
{
	switch (selection)
	{
	case 0: // UINT8
...
void LMS8FE_wxgui::OnLMS8001Close(wxCloseEvent &event)
{
	lms8001GUI->Destroy();
	lms8001GUI = nullptr;
}

void LMS8FE_wxgui::OnmiLMS8001(wxCommandEvent &event)
{
	auto *dev = static_cast<LMS8FE_Device *>(lms8fe);
...
void LMS8FE_wxgui::OnbtnApplyData(wxCommandEvent &event)
{
...
void LMS8FE_wxgui::setStateBit(uint8_t *state, int byte, int bit, int value)
{
	if (value == 0)
		cbi(state[byte], bit);
	else
...
int LMS8FE_wxgui::getStateBit(uint8_t *state, int byte, int bit)
{
	int result;
...
void LMS8FE_wxgui::OnbtnReadData(wxCommandEvent &event)
{
	uint8_t state[STATE_SIZE];

	int result = LMS8FE_Get_Config_Full(lms8fe, state, STATE_SIZE);
	if (result != LMS8FE_SUCCESS)
	{
		PrintError(result);
		return;
...
void LMS8FE_wxgui::OnEraseBackground_pnlChannel(wxEraseEvent &event)
{
	wxDC *dc = event.GetDC();
...
void LMS8FE_wxgui::OnSimpleChanged(wxCommandEvent &event)
{
	Simple2Detailed();
	applied = false;
	btnApplyData->Enable(!applied);
}
void LMS8FE_wxgui::Simple2Detailed()
{
...
void LMS8FE_wxgui::OnDetailedChange(wxCommandEvent &event)
{
	Detailed2Simple();
	applied = false;
	btnApplyData->Enable(!applied);
...
void LMS8FE_wxgui::Detailed2Simple()
{
	int channel = cmbChannel->GetSelection();
