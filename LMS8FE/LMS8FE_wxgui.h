#ifndef __LMS8FE_wxgui__
#define __LMS8FE_wxgui__
...
class LMS8FE_wxgui : public LMS8FE_view
{
protected:
	...
	void State2GUI(lms8fe_boardState state);
	void GUI2State(lms8fe_boardState *state);
	void setTXRXBtns();
	...
	lms8fe_boardState configuredState;

	int activeMode;

	void UpdateLMS8FEForm();
	...
	lms8fe_dev_t *lms8fe; // Port handle
...
public:
	/** Constructor */
	LMS8FE_wxgui(wxWindow *parent, wxWindowID id = wxID_ANY, const wxString &title = wxEmptyString, const wxPoint &pos = wxDefaultPosition, const wxSize &size = wxDefaultSize, long styles = 0);
	void Initialize(lms_device_t *lms);
};

#endif // __LMS8FE_wxgui__
